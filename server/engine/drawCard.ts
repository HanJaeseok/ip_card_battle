import { ANIMALS, SHEEP_SAFETY_CAP, FESTIVAL_EXP_MULTIPLIER } from 'shared';
...
function settleStacks(state: GameState): GameEvent[] {
  ...
  for (const animal of ANIMALS) {
    ...
    const baseExp = uncollected.reduce((sum, c) => sum + c.num, 0);
    const doubled = state.festival;
    const exp = doubled ? baseExp * FESTIVAL_EXP_MULTIPLIER : baseExp;
